...
<section class="mk-wrapper">
    ...
    <div style="margin-bottom:12px;display:flex;align-items:center;gap:12px;">
        <span style="font-size:0.9em;color:#666;">{{ $mahasiswas->count() }} mahasiswa</span>
        @if (auth()->user()->isAdmin() || auth()->user()->isKaprodi())
        <button type="button" class="mk-apply-btn" onclick="openAddMhsModal()">+ Tambah Mahasiswa</button>
        @endif
    </div>
        ...
        <table class="mk-table">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        <span style="padding:2px 8px;border-radius:10px;font-size:0.82em;
                    ...
                    <td>
                        <div class="action-group">
                            ...
                            <a href="{{ route('nilai.index', ['kode_dosen' => $mhs->kode_dosen]) }}" class="btn-manage">Nilai</a>
                        </div>
                    </td>
                </tr>
                @empty
                <tr><td colspan="8" style="text-align:center;">Belum ada data mahasiswa.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
            ...
            <form method="POST" action="{{ route('mahasiswa.store') }}">
                ...
                <div class="edit-modal-body">
                    ...
                    <div class="edit-field-group">
                        ...
                        <input type="number" name="tahun_masuk" required min="2000" max="2100" value="{{ date('Y') }}">
                    </div>
                    <div class="edit-field-group">
                    ...
                    <div class="edit-field-group">
                        ...
                        <select name="kode_dosen">
                            ...
                            @foreach ($dosenList as $d)
                                <option value="{{ $d->kode_dosen }}">{{ $d->kode_dosen }} — {{ $d->nama_lengkap }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
        ...
        <div class="edit-mk-modal" style="max-width:480px;">
            ...
            <form id="editMhsForm" method="POST" action="">
                ...
                <div class="edit-modal-body">
                    ...
                    <div class="edit-field-group">
                        <label>Tahun Masuk</label>
                        <input type="number" id="editTahunMasuk" name="tahun_masuk" required min="2000" max="2100">
                    </div>
                    <div class="edit-field-group">
                        <label>Kode Kelas</label>
...
@section('scripts')
<script>
    function openAddMhsModal()  { document.getElementById('addMhsModal').classList.add('show'); }
    function closeAddMhsModal() { document.getElementById('addMhsModal').classList.remove('show'); }
    ...
    function closeEditMhsModal() { document.getElementById('editMhsModal').classList.remove('show'); }
</script>
@endsection
