    ...
    public function calculate(int $idMahasiswa, array $filters = []): array
    {
        $assessmentScores = DB::table('nilai_mahasiswa')
            ...
            ->select(
                ...
                'mata_kuliah.id_mk', 'mata_kuliah.kode_mk', 'mata_kuliah.nama_matakuliah',
                'mata_kuliah.sks', 'mata_kuliah.semester',
                'assessment_tools.id_at', 'assessment_tools.nama_at', 'assessment_tools.weight_in_clo',
        ...
        foreach ($cloResults as $id => $clo) {
            $cloResults[$id]['final_clo_score'] = $clo['total_weight'] > 0
        ...
        foreach ($ploResults as $ploId => $plo) {
            $ploResults[$ploId]['final_plo_score'] = $plo['total_clo'] > 0
                ? round($plo['total_clo_score'] / $plo['total_clo'], 2)
                : 0;
        }

        return ['clo_results' => $cloResults, 'plo_results' => $ploResults];
    }
}
